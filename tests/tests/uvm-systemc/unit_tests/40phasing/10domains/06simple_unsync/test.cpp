...
// checked. Domain synchronization is done using sync and unsync.
...
class test : public uvm_component
{
 public:
  ...
  : uvm_component(name), leaf1("l1"), leaf2("l2"), domain1("domain1"), domain2("domain2")
  {
    ...
    // Sync domain1 main_phase with domain2 post_reset_phase
    // The result is that leaf1 and leaf2 run independently (as if no sync was defined)
  ...
  void report_phase(uvm_phase& phase) override
  {
    ...
    if( leaf1.start_main != sc_time(200, SC_SEC) || leaf1.end_main != sc_time(400, SC_SEC))
    {
      std::cout << "*** UVM TEST FAILED, leaf1 main " << leaf1.start_main
                << " - " << leaf1.end_main << ", expected 200 - 400 ***" << std::endl;
      failed = true;
    }

    if( leaf1.start_shutdown != sc_time(400, SC_SEC) || leaf1.end_shutdown != sc_time(600, SC_SEC))
    {
      std::cout << "*** UVM TEST FAILED, leaf1 shutdown " << leaf1.start_shutdown
                << " - " << leaf1.end_shutdown << ", expected 400 - 600 ***" << std::endl;
      failed = true;
    }
    ...
    if( leaf2.start_main != sc_time(250, SC_SEC) || leaf2.end_main != sc_time(500, SC_SEC))
    {
      std::cout << "*** UVM TEST FAILED, leaf2 main " << leaf2.start_main
                << " - " << leaf2.end_main << ", expected 250 - 500 ***" << std::endl;
      failed = true;
    }

    if( leaf2.start_shutdown != sc_time(500, SC_SEC) || leaf2.end_shutdown != sc_time(750, SC_SEC))
    {
      std::cout << "*** UVM TEST FAILED, leaf2 shutdown " << leaf2.start_shutdown
                << " - " << leaf2.end_shutdown << ", expected 500 - 750 ***" << std::endl;
      failed = true;
    }
