...
class my_driver : public uvm::uvm_driver<REQ,RSP>
{
 public:
  ...
  void run_phase(uvm::uvm_phase& phase)
  {
    for(;;) // forever loop
    {
      // try next item will return an empty handle if there is no item
      // available
      auto req = this->seq_item_port->try_next_item();

      // only when item was valid, we continue processing...
      if (req)
      {
        ...
        this->seq_item_port->item_done();       // or alternative this->seq_item_port->get() to flush item from fifo
